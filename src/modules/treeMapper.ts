...
const createMUIStructureFromTree = (
    pathsTree: Tree,
...
export const mapFilesystemPathsToMUIRichTreeFormat = (
    ...
    directoryPaths.forEach((path) => {
        ...
        let nextFoundTreeForDirectory = pathsTrees.find(
            (tree) => tree?.value === nestedDirectories[currentLevel],
        );
        ...
        while (foundExistingTreeForDirectory && nextFoundTreeForDirectory) {
            ...
            nextFoundTreeForDirectory = pathsTrees.find(
                (tree) => tree?.value === nestedDirectories[currentLevel],
            );
        }
        ...
        if (isTree(foundExistingTreeForDirectory)) {
            ...
            const leafNode = new Tree(
                nestedDirectories.slice(currentLevel),
                currentLevel,
            );
